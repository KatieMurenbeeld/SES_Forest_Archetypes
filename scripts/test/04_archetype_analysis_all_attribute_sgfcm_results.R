...
SGFCM_all_result <- SGFCMeans(dataset, k = 8, m = 1.8, standardize = FALSE,
                              lag_method = "mean",
                              window = w1, alpha = 1.3, beta = 0.3,
                              seed = 6891, tol = 0.001, verbose = TRUE, init = "kpp")

map_SGFCM_result <- rast(SGFCM_all_result$rasters)
plot(map_SGFCM_result[["Groups"]])
writeRaster(map_SGFCM_result[["Groups"]], filename = paste0("/Users/katiemurenbeeld/Analysis/Archetype_Analysis/outputs/SGFCM_all_result_", 
                                                            Sys.Date(), ".tif"))
