...
idw_preds <- function(data_proj, ref_raster, lay, empty_grid){
  var.rst <- rasterize(data_proj, ref_raster, field = lay, fun = "mean") # this will turn things into NA if it is split
...
plot(aip_crop)
#writeRaster(aip_crop, paste0("/Users/katiemurenbeeld/Analysis/Archetype_Analysis/data/processed/aip_3km_pred_crop_", 
#                                         Sys.Date(), ".tif"))


## need to test something out so I can use focalPairs()
test_aip.rst <- rasterize(aip_proj, ref_rast_proj, field = "mrp_ideology", 
                          fun = "mean")
test_aip.rst <- crop(test_aip.rst, ref_rast_proj, mask = TRUE)
plot(test_aip.rst, colNA="red")
