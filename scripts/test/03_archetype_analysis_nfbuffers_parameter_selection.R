...
R2s <- sapply(2:50, function(k){
  Clust <- k_means(df_nat_comm, centers=k, iter.max = 150)
...
k_r2
ggsave(here::here(paste0("outputs/plots/nf_level_rules_param_selection_kmeans_r2_k100_", 
                         Sys.Date(), ".jpeg")), 
...
INERTs <- sapply(2:100,function(k){
  Clust <- kmeans(df_nat_rules, centers=k, iter.max = 150)
...
k_inert
ggsave(here::here(paste0("outputs/plots/nf_level_rules_param_selection_kmeans_inertia_k100_", 
                         Sys.Date(), ".jpeg")), 
...
fcm_xb <- ggplot(FCMvalues) + 
  ...
  geom_text(aes(x = k, y = m, label = round(XieBeni.index,0)), size = 2)+
  scale_fill_viridis() +
  coord_fixed(ratio=10) +
  labs(title = "Xie Beni") +
  theme(legend.position = "NULL")
