...
sgfcm_soc_k3_result <- rast("/Users/katiemurenbeeld/Analysis/Archetype_Analysis/outputs/SGFCM_soc_result_k3_2024-10-22.tif")
sgfcm_soc_k6_result <- rast("/Users/katiemurenbeeld/Analysis/Archetype_Analysis/outputs/SGFCM_soc_result_k6_2024-10-22.tif")
...
fs_nf.crop <- st_crop(fs_nf.proj, ext(sgfcm_soc_k6_result))
fs_reg.proj <- fs_reg %>% 
  filter(REGION != "10") %>%
  st_transform(., crs=projection)
fs_reg.crop <- st_crop(fs_reg.proj, ext(sgfcm_soc_k6_result))
...
z_k3 <- crop(sgfcm_soc_k3_result, v, mask = TRUE)
...
# repeat for k = 6
z_k6 <- crop(sgfcm_soc_k6_result, v, mask = TRUE)
...
shan_h_soc_sf <- shan_h_k3 %>%
  left_join(fs_nf.crop, by = "FORESTORGC")
shan_h_soc_sf <- left_join(shan_h_soc_sf, shan_h_k6, by = "FORESTORGC")
shan_h_soc_sf <- st_as_sf(shan_h_soc_sf)

# save the shape file
write_sf(shan_h_soc_sf, here::here(paste0("data/processed/shan_h_soc_nf_", 
                                      Sys.Date(), ".shp")), overwrite = TRUE)

# make maps
shan_soc_k3_nf <- ggplot() +
  geom_sf(data = fs_reg.crop, fill = NA, color = "black", linewidth = 0.75) +
  geom_sf(data = shan_h_soc_sf, aes(fill = shan_div_sc_k3)) +
  labs(title = "Shannon Entropy (H) of Soc Archetypes (k = 3)",
...
#shan_soc_k3_nf
ggsave(here::here(paste0("outputs/plots/shan_div_soc_k3_nf_", Sys.Date(), ".png")),
       plot = shan_soc_k3_nf, width = 7, height = 5, dpi = 300)

shan_soc_k6_nf <- ggplot() +
  geom_sf(data = fs_reg.crop, fill = NA, color = "black", linewidth = 0.75) +
  geom_sf(data = shan_h_soc_sf, aes(fill = shan_div_sc_k6)) +
  labs(title = "Shannon Entropy (H) of Soc Archetypes (k = 6)",
       subtitle = "Calculated from 50km buffer around National Forests") +
  scale_fill_viridis(limits = c(0, 1)) +
  theme_bw() + 
  theme(text = element_text(size = 20),
        ...
        plot.margin=unit(c(0.5, 0.5, 0.5, 0.5),"mm"))
#shan_soc_k6_nf
ggsave(here::here(paste0("outputs/plots/shan_div_soc_k6_nf_", Sys.Date(), ".png")),
       plot = shan_soc_k6_nf, width = 7, height = 5, dpi = 300)

# calculate the shannon diversity assuming crips archetypes for each Region 
z_reg_k3 <- crop(sgfcm_soc_k3_result, fs_reg.crop, mask = TRUE)
...
# k = 6
z_reg_k6 <- crop(sgfcm_soc_k6_result, fs_reg.crop, mask = TRUE)
...
shan_h_soc_reg_sf <- shan_h_reg_k3 %>%
  left_join(fs_reg.crop, by = "REGION")
shan_h_soc_reg_sf <- left_join(shan_h_soc_reg_sf, shan_h_reg_k6, by = "REGION")
shan_h_soc_reg_sf <- st_as_sf(shan_h_soc_reg_sf)

# save the shape file
write_sf(shan_h_soc_reg_sf, here::here(paste0("data/processed/shan_h_soc_reg_", 
                                      Sys.Date(), ".shp")), overwrite = TRUE)

# make maps
shan_reg_k3 <- ggplot() +
  geom_sf(data = shan_h_soc_reg_sf, aes(fill = shan_div_sc_k3)) +
  labs(title = "Shannon Entropy (H) of Soc Archetypes k = 3",
  ...
  theme(text = element_text(size = 20),
        ...
        plot.margin=unit(c(0.5, 0.5, 0.5, 0.5),"mm"))
#shan_reg_k3
ggsave(here::here(paste0("outputs/plots/shan_div_soc_reg_k3_", Sys.Date(), ".png")),
       plot = shan_reg_k3, width = 7, height = 5, dpi = 300)

# k = 6
shan_reg_k6 <- ggplot() +
  geom_sf(data = shan_h_soc_reg_sf, aes(fill = shan_div_sc_k6)) +
  labs(title = "Shannon Entropy (H) of Soc Archetypes k = 6",
       subtitle = "Calculated for each Region") +
  scale_fill_viridis(limits = c(0, 1)) +
  theme_bw() + 
  theme(text = element_text(size = 20),
        ...
        plot.margin=unit(c(0.5, 0.5, 0.5, 0.5),"mm"))
#shan_reg_k6
ggsave(here::here(paste0("outputs/plots/shan_div_soc_reg_k6_", Sys.Date(), ".png")),
